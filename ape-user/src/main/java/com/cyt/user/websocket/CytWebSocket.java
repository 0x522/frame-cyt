package com.cyt.user.websocket;

import com.cyt.config.WebSocketServerConfig;
...
@Slf4j
@ServerEndpoint(value = "/cyt/socket", configurator = WebSocketServerConfig.class)
@Component
public class CytWebSocket {
     ...
     * 存放所有在线的客户端
     */
    private static Map<String, CytWebSocket> clients = new ConcurrentHashMap<>();
    ...
    public void onMessage(String message, Session session) {
        log.info("服务端收到客户端[{}]的消息:{},当前会话{}", this.erp, message, session);
        //心跳机制
    ...
    public void sendMessage(String message) {
        for (Map.Entry<String, CytWebSocket> sessionEntry : clients.entrySet()) {
            String erp = sessionEntry.getKey();
            CytWebSocket socket = sessionEntry.getValue();
